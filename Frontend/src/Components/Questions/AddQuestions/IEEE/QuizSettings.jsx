...
const QuizSettings = ({ totalMarks, totalQuestions }) => {
  ...
  const fetchQuizSettings = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://clubportal-3s4b.onrender.com/questions/ieee/${sig}/settings`, {
        method: 'GET',
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`https://clubportal-3s4b.onrender.com/questions/ieee/${sig}/settings/update`, {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${userData.token}`
