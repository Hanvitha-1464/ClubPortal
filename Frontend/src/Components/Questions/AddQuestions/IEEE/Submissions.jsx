  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      try {
        ...
        const response = await fetch(`https://clubportal-3s4b.onrender.com/results/ieee/${sig}/submissions`, {
          method: 'GET',
          headers: {
  ...
  const saveAllChanges = async () => {
    ...
    try {
      ...
      const updatePromises = Object.entries(modifiedSubmissions).map(([submissionId, changes]) => {
        return fetch(`https://clubportal-3s4b.onrender.com/results/ieee/${sig}/submissions/${submissionId}/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${userData.token}`
